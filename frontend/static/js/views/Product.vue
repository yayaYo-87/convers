<template>
  <div class="product">
    <div class="product__header">
      Home » Products
    </div>
    <h1 class="product__title">
      Продукты
    </h1>
    <div class="product__items">
      <div class="product_item">
        <router-link tag="div" :to="{ name: 'cart' }" class="product_item-img">
          <div class="product_item-img_wrapper">
            <img class="product_item-img_one" src="../../img/5.png" alt="">
            <img class="product_item-img_two" src="../../img/6.jpg" alt="">
          </div>
        </router-link>
        <div class="product_item-flex">
          <div class="product_item-title">
            <span>Classical Conversations 20th Anniversary Poster</span>
          </div>
          <div class="product_item-bottom">
            <div class="product_item-bottom_price">
              $10.00
            </div>
            <button class="product_item-bottom_button">
              <span>Купить</span>
            </button>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
  import axios from 'axios'

  export default {
    data() {
      return {
        result: []
      }
    },
    methods: {
      get() {
        const self = this
        axios.get('/api/category/')
      }
    }
  }
</script>
